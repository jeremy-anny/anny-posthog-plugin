...
Add your `Web snippet` (which you can find in [your project settings](https://posthog.anny.cloud/settings/project#snippet)) in the `<header>` of your `web/index.html` file. Write your project token into the snippet as a literal string. It's public, the same token ships to every visitor, and it needs no build-time or deploy-time injection:
...
To set up [session replay web](/docs/session-replay.md) or [mobile session replay](/docs/session-replay/mobile.md) in your project, all you need to do is install the Flutter SDK, follow the [additional installation instructions](/docs/session-replay/installation/flutter.md), and enable "Record user sessions" in [your project settings](https://posthog.anny.cloud/settings/project-replay) and enable the `sessionReplay` option.

If you're using Flutter Web, also enable the [Canvas capture](/docs/session-replay/canvas-recording.md) in [your project settings](https://posthog.anny.cloud/settings/project-replay). This is needed as Flutter renders your app using a browser canvas element.
