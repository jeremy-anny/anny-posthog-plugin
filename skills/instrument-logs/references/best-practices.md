...
Automatic PII scrubbing is currently available to internal PostHog teams only while we measure its ingestion overhead. If you'd like early access, please [reach out to us](https://posthog.anny.cloud/project/2/settings/#panel=support%3Asupport%3A%3A%3Afalse) via in-app support.

Once you have access, enable it under [**Project settings** → **Logs** → **PII scrubbing**](https://posthog.anny.cloud/settings/environment-logs#logs-pii-scrub). The toggle is off by default.
