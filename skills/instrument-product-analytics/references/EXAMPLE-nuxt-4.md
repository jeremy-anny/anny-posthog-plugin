...
Get your PostHog project token from your [PostHog project settings](https://posthog.anny.cloud/project/settings).

For source map uploads, get your project ID from [PostHog environment variables](https://posthog.anny.cloud/settings/environment#variables) and your personal API key from [PostHog user API keys](https://posthog.anny.cloud/settings/user-api-keys) (requires `organization:read` and `error_tracking:write` scopes).
...
export default defineNuxtConfig({
  ...
  posthogConfig: {
    publicKey: process.env.NUXT_PUBLIC_POSTHOG_PROJECT_TOKEN || '', // Find it in project settings https://posthog.anny.cloud/settings/project
    ...
    sourcemaps: {
      enabled: true,
      envId: process.env.PROJECT_ID || '', // Your project ID from PostHog settings https://posthog.anny.cloud/settings/environment#variables
      personalApiKey: process.env.PERSONAL_API_KEY || '', // Your personal API key from PostHog settings https://posthog.anny.cloud/settings/user-api-keys (requires organization:read and error_tracking:write scopes)
